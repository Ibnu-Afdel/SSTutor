<div class="px-4 py-10 mx-auto max-w-7xl sm:px-6 lg:px-8"> 
    ...
    @if (session()->has('message'))
        <div class="flex items-center p-4 mb-6 text-sm text-green-700 bg-green-100 border border-green-200 rounded-lg shadow-sm" role="alert">
            <i class="mr-2 fas fa-check-circle fa-fw"></i>
            <span>{{ session('message') }}</span>
        </div>
    @endif
    @if (session()->has('error'))
         <div class="flex items-center p-4 mb-6 text-sm text-red-700 bg-red-100 border border-red-200 rounded-lg shadow-sm" role="alert">
    ...
    @if (!$isCreatingOrEditing)
        @if ($courses->count())
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"> 
                ...
                <div class="flex flex-col overflow-hidden transition duration-150 ease-in-out bg-white border border-gray-200 shadow-md rounded-xl hover:shadow-lg hover:border-indigo-300">
